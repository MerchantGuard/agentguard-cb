# Third-Party Notices

`@merchantguard/agentguard-cb` redistributes the following third-party
production dependencies. Each is included subject to the terms of its own
license. Generated via `npx license-checker --production`.

| Package | Version | License | Repository |
|---|---|---|---|
| @next/env | 14.2.35 | MIT | https://github.com/vercel/next.js |
| @noble/ed25519 | 2.3.0 | MIT | https://github.com/paulmillr/noble-ed25519 |
| @pdf-lib/standard-fonts | 1.0.0 | MIT | https://github.com/Hopding/standard-fonts |
| @pdf-lib/upng | 1.0.1 | MIT | https://github.com/Hopding/upng |
| @swc/counter | 0.1.3 | Apache-2.0 | https://github.com/swc-project/pkgs |
| @swc/helpers | 0.5.5 | Apache-2.0 | https://github.com/swc-project/swc |
| @types/node | 20.19.39 | MIT | https://github.com/DefinitelyTyped/DefinitelyTyped |
| @types/prop-types | 15.7.15 | MIT | https://github.com/DefinitelyTyped/DefinitelyTyped |
| @types/react | 18.3.28 | MIT | https://github.com/DefinitelyTyped/DefinitelyTyped |
| busboy | 1.6.0 | MIT | https://github.com/mscdex/busboy |
| caniuse-lite | 1.0.30001791 | CC-BY-4.0 | https://github.com/browserslist/caniuse-lite |
| client-only | 0.0.1 | MIT | — |
| csstype | 3.2.3 | MIT | https://github.com/frenic/csstype |
| drizzle-orm | 0.36.4 | Apache-2.0 | https://github.com/drizzle-team/drizzle-orm |
| graceful-fs | 4.2.11 | ISC | https://github.com/isaacs/node-graceful-fs |
| js-tokens | 4.0.0 | MIT | https://github.com/lydell/js-tokens |
| loose-envify | 1.4.0 | MIT | https://github.com/zertosh/loose-envify |
| nanoid | 3.3.11 | MIT | https://github.com/ai/nanoid |
| next | 14.2.35 | MIT | https://github.com/vercel/next.js |
| pako | 1.0.11 | MIT AND Zlib | https://github.com/nodeca/pako |
| pdf-lib | 1.17.1 | MIT | https://github.com/Hopding/pdf-lib |
| picocolors | 1.1.1 | ISC | https://github.com/alexeyraspopov/picocolors |
| postcss | 8.4.31 | MIT | https://github.com/postcss/postcss |
| postgres | 3.4.9 | Unlicense | https://github.com/porsager/postgres |
| react | 18.3.1 | MIT | https://github.com/facebook/react |
| react-dom | 18.3.1 | MIT | https://github.com/facebook/react |
| scheduler | 0.23.2 | MIT | https://github.com/facebook/react |
| source-map-js | 1.2.1 | BSD-3-Clause | https://github.com/7rulnik/source-map-js |
| streamsearch | 1.1.0 | MIT | https://github.com/mscdex/streamsearch |
| stripe | 22.1.0 | MIT | https://github.com/stripe/stripe-node |
| styled-jsx | 5.1.1 | MIT | https://github.com/vercel/styled-jsx |
| tslib | 1.14.1 / 2.8.1 | 0BSD | https://github.com/Microsoft/tslib |
| undici-types | 6.21.0 | MIT | https://github.com/nodejs/undici |
| zod | 3.25.76 | MIT | https://github.com/colinhacks/zod |

## License compatibility

All listed licenses are compatible with the project's MIT License under the
SPDX compatibility matrix. Apache-2.0 components carry their own attribution
and patent-grant requirements; this file constitutes the required NOTICE
attribution for the redistributed Apache-2.0 components above (`@swc/counter`,
`@swc/helpers`, `drizzle-orm`).

## Regenerating this file

```bash
npx license-checker --production --csv > THIRD-PARTY-NOTICES.csv
# then update the table above
```

Last updated: 2026-05-01.
